...
st.markdown("""
    ...
    /* Абсолютное принудительное выравнивание для текстовых блоков ввода */
    ...
    /* Песочный фон для текстовых инпутов журнала */
    .journal-input input, .journal-input textarea {
        background-color: #fdf5e6 !important;
        color: black !important;
    }

    /* Голубой фон для задач */
    .jobs-input input {
        background-color: #ebf5fb !important;
        color: black !important;
    }
    
    .stTabs [data-baseweb="tab-list"] { background-color: #7a8594; border-radius: 5px; padding: 2px; }
    .stTabs [data-baseweb="tab"] { font-size: 22px !important; font-weight: bold !important; color: white !important; }
    .stTabs [aria-selected="true"] { background-color: #2c3e50 !important; color: #fff !important; }
    .stButton button[kind="primary"] { background-color: #28a745 !important; color: white !important; font-weight: bold; font-size: 18px; }
    </style>
...
# ==========================================
# ОКНО 1: ОПЕРАТИВНЫЙ ЖУРНАЛ (БЕЗ ДАТА-ЭДИТОРА)
# ==========================================
with tab_log:
    ...
    for u_name, u_num in units:
        ...
        with c_morn:
            st.markdown(f'<div style="background-color:#d35400; color:white; padding:4px; text-align:center; font-weight:bold; border:2px solid black;">{u_num}. {u_name} - משמרת בוקר</div>', unsafe_allow_html=True)
            for idx in range(6):
                col_d, col_h = st.columns([4, 1])
                with col_h:
                    h_val = st.text_input("שעה", value=m_data[idx].get('Hour',''), key=f"h_m_{u_num}_{idx}_{date_str}", label_visibility="collapsed")
                with col_d:
                    st.markdown('<div class="journal-input">', unsafe_allow_html=True)
        ...
        with c_night:
            st.markdown(f'<div style="background-color:#2980b9; color:white; padding:4px; text-align:center; font-weight:bold; border:2px solid black;">{u_num}. {u_name} - משמרת לילה</div>', unsafe_allow_html=True)
            for idx in range(6):
                col_d, col_h = st.columns([4, 1])
                with col_h:
...
# ==========================================
# ОКНО 2: РАСПИСАНИЕ (СТАТИЧЕСКИЙ ВЬЮВЕР С ЦВЕТОМ)
# ==========================================
with tab_sch:
...
# ==========================================
# ОКНО 3: РАБОТЫ НА СЕГОДНЯ (ФИЗИЧЕСКИЙ RTL ВВОД)
# ==========================================
with tab_jobs:
    st.markdown("<h3>עבודות מתוכננות להיום</h3>", unsafe_allow_html=True)
    
    loaded_jobs = load_jobs_data_list()
    while len(loaded_jobs) < 15: loaded_jobs.append("")
    
    saved_jobs_inputs = []
    
    # Строим 15 полей ввода вручную без использования data_editor
    for i in range(15):
        col_task, col_num = st.columns([12, 1])
        with col_num:
            st.markdown(f'<p style="font-size:18px; font-weight:bold; text-align:right; margin-top:6px;">{i+1}</p>', unsafe_allow_html=True)
        with col_task:
            st.markdown('<div class="jobs-input">', unsafe_allow_html=True)
